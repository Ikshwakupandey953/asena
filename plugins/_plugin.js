...
Asena.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC}, (async (message, match) => {
    ...
    if (response.statusCode == 200) {
        ...
        var find = ''
        var sp = plugins.map((plugin) => { find += plugin.dataValues.name })
        if (find.includes(plugin_name)) {
            await message.sendMessage('*Aynı Plugini 2 Defa Yükleyemezsin!*')
            await new Promise(r => setTimeout(r, 400));
            fs.unlinkSync('/root/WhatsAsenaDuplicated/plugins/' + plugin_name + '.js')
        } else if (response.body.includes('formation')) {
            await message.sendMessage('*Bu Plugin Zararlı Olduğundan Yüklenemez!*')
            await new Promise(r => setTimeout(r, 400));
            fs.unlinkSync('/root/WhatsAsenaDuplicated/plugins/' + plugin_name + '.js')
        } else {
            await new Promise(r => setTimeout(r, 400));
